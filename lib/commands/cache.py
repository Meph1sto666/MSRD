...
def clear(
    type: Annotated[
        str,
        typer.Argument(
            help="The kind of cache to clear ['audio', 'lyrics', 'cover' or 'all']"
        ),
    ],
    y: Annotated[
        bool, typer.Option("--yes", "-y", help="Assume yes for the confirm request.")
    ] = False,
) -> None:
    if not type in ["audio", "lyrics", "cover"]:
        typer.pause(
            f"Specified cache ({type.upper()}) does not exist. Press any key to continue..."
        )
        return
    if not os.path.exists(os.getenv(f"{type.upper()}_CACHE_DIR", "")):
        typer.pause(
            f"{type.upper()} cache is already cleared. Press any key to continue..."
        )
        return
    if not y:
