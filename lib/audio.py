...
def _has_libfdk_aac() -> bool:
	"""Check if libfdk_aac is available (best AAC encoder, but hard to install).
...
class Song:
	...
	def is_downloaded(self) -> bool:
		return os.path.exists(
			f"./{os.getenv(self.__target_codec.upper() + '_LIBRARY_DIR')}/{self.song_cid}{self.__target_ext}"
		)
	...
	def convert(self) -> None:
		...
		if self.__audio_codec not in [".flac", ".wav"]:
			pbar.write(
				f"{colorama.Fore.LIGHTBLUE_EX}INFO{colorama.Fore.RESET}: "
				f"[{self.song_cid}] {colorama.Fore.LIGHTRED_EX}ORIGINAL IS NOT LOSSLESS{colorama.Fore.RESET}"
			)
	...
	def full_download(self) -> None:
		...
		library_dir = f"./{os.getenv(self.__target_codec.upper() + '_LIBRARY_DIR')}/"
		os.makedirs(library_dir, exist_ok=True)
		os.replace(
			f"./{CONVERTED_CACHE}{self.song_cid}{self.__target_ext}",
			f"{library_dir}{self.song_cid}{self.__target_ext}",
		)
