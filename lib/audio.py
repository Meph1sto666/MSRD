...
def _request_song_info(song_id: str | None = None) -> dict[str, typing.Any]:
    return requests.get(
        f"https://monster-siren.hypergryph.com/api/song/{song_id}"
    ).json()["data"]


def _request_album_info(album_id: str | None = None) -> dict[str, typing.Any]:
    return requests.get(
        f"https://monster-siren.hypergryph.com/api/album/{album_id}/detail"
    ).json()["data"]


def get_song_list() -> list[dict[str, typing.Any]]:
    return (
        requests.get("https://monster-siren.hypergryph.com/api/songs")
        .json()["data"]
        .get("list")
    )
...
class Song:
    def __init__(
        ...
        target_codec: typing.Literal["flac", "m4a", "mp3"] = "flac",
    ) -> None:
        __song_dta: dict[str, typing.Any] = _request_song_info(song_id)
        __album_dta: dict[str, typing.Any] = _request_album_info(
            __song_dta.get("albumCid")
        )
        ...
        self.__audio_codec: str | None = (
            os.path.splitext(self.song_url)[1] if self.song_url else None
        )
        self.__cover_codec: str = os.path.splitext(
            self.song_mv_cover_url or self.album_cover_url
        )[1]
        self.__lyrics_ext: str | None = (
            os.path.splitext(self.lyrics_url)[1] if self.lyrics_url else None
        )
    ...
    def download_song(self) -> None:
        ...
        with open(path, "wb") as audio_file:
            ...
            pbar = tqdm.tqdm(
                unit="B",
                total=int(audio_stream.headers["Content-Length"]),
                desc=f"Downloading [{self.song_cid}{self.__audio_codec}]",
                ascii=".#",
                unit_scale=True,
                leave=False,
            )  # , bar_format="{rate} {elapsed} {desc} |{bar}| {percentage:.2f}%"
    ...
    def download_cover(self) -> None:
        ...
        with open(path, "wb") as cover_file:
            cover_stream: requests.Response = requests.get(
                self.song_mv_cover_url or self.album_cover_url, stream=True
            )
            pbar = tqdm.tqdm(
                unit="B",
                total=int(cover_stream.headers["Content-Length"]),
                desc=f"Downloading [{self.song_cid}{self.__cover_codec}]",
                ascii=".#",
                unit_scale=True,
                leave=False,
            )  # , bar_format="{rate} {elapsed} {desc} |{bar}| {percentage:.2f}%"
    ...
    def download_lyrics(self) -> None:
        ...
        with open(path, "wb") as lrc_file:
            ...
            pbar = tqdm.tqdm(
                unit="B",
                total=int(lrc_stream.headers["Content-Length"]),
                desc=f"Downloading [{self.song_cid}{self.__lyrics_ext}]",
                ascii=".#",
                unit_scale=True,
                leave=False,
            )  # , bar_format="{rate} {elapsed} {desc} |{bar}| {percentage:.2f}%"
    ...
    def convert(self) -> None:
        ...
        progress_iterator: typing.Iterator[float] = FfmpegProgress(
            cmd
        ).run_command_with_progress()  # type: ignore
        pbar = tqdm.tqdm(
            progress_iterator,
            total=100,
            desc=f"Converting [{self.song_cid}]",
            ascii=".#",
            unit_scale=True,
            leave=False,
        )
        if self.__audio_codec not in [".flac", ".wav"]:
            pbar.write(
                f"{colorama.Fore.LIGHTBLUE_EX}INFO{colorama.Fore.RESET}: [{self.song_cid}] {colorama.Fore.LIGHTRED_EX}ORIGINAL IS NOT LOSSLESS{colorama.Fore.RESET}"
            )
        list(pbar)

    def add_metadata_flac(self) -> None:
        pbar = tqdm.tqdm(
            total=1, desc=f"Adding metadata [{self.song_cid}]", ascii=".#", leave=False
        )
        cover = Picture()
    ...
    def add_metadata_mp3(self) -> None:
        ...
        with open(f"{COVER_CACHE}{self.song_cid}{self.__cover_codec}", "rb") as f:
            ...
            audio_file_cvr.tags.add(
                APIC(
                    mime=f"image/{'png' if self.__cover_codec == '.png' else 'jpeg'}",
                    type=3,
                    desc="Cover",
                    data=f.read(),
                )
            ...
            tag.setall(
                ...
                [
                    USLT(
                        ...
                        format=2,
                        type=1,
                        text=open(lrc_path, "r", encoding="utf-8").read(),
                    )
                ],
            )  # type: ignore
    ...
    def full_download(self) -> None:
        ...
        os.makedirs(
            f"./{os.getenv(self.__target_codec.upper() + '_LIBRARY_DIR')}/",
            exist_ok=True,
        )
        os.replace(
            f"./{CONVERTED_CACHE}{self.song_cid}.{self.__target_codec}",
